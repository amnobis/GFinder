...
public class AutoCompleteComboBox<T> {
    ...
    private void selectClosestResultBasedOnTextFieldValue(boolean affect, boolean inFocus) {
        ObservableList items = AutoCompleteComboBox.this.comboBox.getItems();
        boolean found = false;
        for (int i=0; i<items.size(); i++) {
            if (items.get(i) != null && AutoCompleteComboBox.this.comboBox.getEditor().getText() != null &&
                    AutoCompleteComboBox.this.comboBox.getEditor().getText().toLowerCase().equals(items.get(i).toString().toLowerCase())) {
                try {
                    ListView lv = ((ComboBoxListViewSkin) AutoCompleteComboBox.this.comboBox.getSkin()).getListView();
